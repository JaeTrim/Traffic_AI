...
export default function AdminControlPanel() {
    ...
    const makeAdmin = async () => {
        ...
        try {
            ...
            setSnackbarMessage(`User "${usernameToPromote}" promoted to admin successfully`)
    ...
    const revokeAdmin = async () => {
        ...
        try {
            ...
            setSnackbarMessage(`Admin privileges revoked from "${userToRevoke.username}"`)
            setSnackbarSeverity('success')
            setSnackbarOpen(true)
    ...
    const filteredUsers = searchUsername 
        ? users.filter(user => user.username.toLowerCase().includes(searchUsername.toLowerCase()))
        : users
    ...
    return (
        <Box sx={{ 
                                        ...
                                        {filteredUsers.length > 0 ? (
                                                ...
                                                >
                                                    ...
                                                    <TableCell>
                                                        <Chip 
                                                            ...
                                                            color={user.role === 'admin' ? 'black' : 'default'}
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={3} align="center" sx={{ py: 4 }}>
                                                    {searchUsername ? 'No users found matching your search.' : 'No users found.'}
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
            ...
            <Dialog 
                ...
                PaperProps={{ sx: { borderRadius: 2, p: 1 } }}
            >
                <DialogTitle sx={{ color: '#d32f2f' }}>Remove Admin Privileges</DialogTitle>
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to remove admin privileges from user "{userToRevoke?.username}"?
                    </DialogContentText>
                </DialogContent>
                <DialogActions sx={{ p: 2 }}>
                    <Button 
